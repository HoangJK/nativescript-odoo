...
export class OdooClient {
	...
	private setCurrentUser(user: OdooUser) {
		localStorage.setItem(
			OdooLocalStorageKey.CURRENT_USER,
			JSON.stringify(user)
		);
	}
	...
	public authenticate(userName: string, password: string, db: string, optional?: any) {
		...
		return this.httpRequestPost({
			...
			.then((sessionInfo: any) => {
				console.log("sessionInfo ", sessionInfo.content.toJSON());
				let user: OdooUser;
				if (sessionInfo) {
					sessionInfo = sessionInfo.content.toJSON().result;
	...
	private getSessionInfo() {
		// let httpRequest = this.httpRequestPost({
